...
struct lpc_dev lpc_devs[] = {
	...
	{0x095c802b, 1112,  16,  4, &lpc_cortex},
	{0x295c802b, 1112,  16,  4, &lpc_cortex},
	{0x097a802b, 1113,  24,  4, &lpc_cortex},
	{0x297a802b, 1113,  24,  4, &lpc_cortex},
	{0x0998802b, 1114,  32,  4, &lpc_cortex},
	{0x2998802b, 1114,  32,  4, &lpc_cortex},
	{0x2954402b, 1122,  16,  6, &lpc_cortex},
	{0x2972402b, 1123,  24,  6, &lpc_cortex},
	{0x2998402b, 1124,  32,  6, &lpc_cortex},
	{0x2988402b, 1124,  32,  6, &lpc_cortex},
	{0x2980002b, 1124,  32,  8, &lpc_cortex},
	{0x0003d440, 1134,  40,  8, &lpc_cortex},
	{0x0001cc40, 1134,  48,  8, &lpc_cortex},
	{0x0001bc40, 1135,  64,  8, &lpc_cortex},
	{0x00019c40, 1136,  96,  8, &lpc_cortex},
	{0x00017c40, 1137, 128,  8, &lpc_cortex},
...
char *lpc_fd_isp_version(int fd)
{
	static char reply[12], *r;
	int minor, major;

	r = lpc_write_c(fd, "K\r\n", 2);
	if (!r || r[0] != '0')
		return NULL;
	if (lpc_fd_gets(fd, reply, 12) < 0)
		return NULL;
	minor = reply[0];
	if (lpc_fd_gets(fd, reply, 12) < 0)
		return NULL;
	major = reply[0];
	sprintf(reply, "%c.%c", major, minor);
	return reply;
}
